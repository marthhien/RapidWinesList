...
public class FileDialog {
		...
		private FileDialogListener m_SimpleFileDialogListener = null;
		...
		//	m_sdcardDirectory = Environment.getExternalStorageDirectory()+File.separator+"Download";
		...
		/*	if (Select_type == FileOpen    ) m_titleView1.setText("Import from :");
			if (Select_type == FileSave    ) m_titleView1.setText("Export to :");
		...
		/*	if (Select_type == FolderChoose || Select_type == FileSave)
			...
			if (Select_type == FileOpen || Select_type == FileSave)
			{
				TextView tvspace = new TextView(m_context);
				tvspace.setText(" ");
				titleLayout.addView(tvspace);
